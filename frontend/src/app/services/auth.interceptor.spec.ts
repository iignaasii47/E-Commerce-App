...
import { provideHttpClient, HttpHandlerFn } from '@angular/common/http';
import { HttpTestingController, provideHttpClientTesting } from '@angular/common/http/testing';
import { HttpRequest, HttpErrorResponse, HttpEvent, HttpResponse } from '@angular/common/http';
...
describe('authInterceptor', () => {
  ...
  it('should add Authorization header when token exists', () => {
    ...
    const nextSpy = vi.fn().mockReturnValue(of(new HttpResponse({ body: {} })));
  ...
  it('should attempt token refresh on 401 and retry with new token', () => {
    ...
    const nextSpy = vi.fn().mockImplementation((r: HttpRequest<unknown>) => {
      ...
      return of(new HttpResponse({ body: { data: 'retried' } }));
    });
    ...
    expect(result).toEqual(new HttpResponse({ body: { data: 'retried' } }));
  ...
  it('should queue concurrent 401 requests while refreshing', () => {
    ...
    const nextSpy = vi.fn().mockImplementation((r: HttpRequest<unknown>) => {
      ...
      return of(new HttpResponse({ body: { url: r.url } }));
    });
